...
# def matmul(a, b):
#     filename = ''.join(random.choices(string.ascii_lowercase + string.digits, k=7)) + str(time.time())
...
class DistributedMiningSampler(DistributedSampler):
...
class DistributedMiningSamplerVigor(DistributedSampler):
    def __init__(self, dataset: Dataset, num_replicas: Optional[int] = None,
                 rank: Optional[int] = None, shuffle: bool = False,
                 seed: int = 0, drop_last: bool = False, batch_size: int = 128, mode = 'similarity', dim=1000, save_path=None) -> None:
        ...
        self.current_indices = np.arange(self.current_size)
        self.queue_size = 1 # for computing moving average, not used in this implementation
        self.queue = np.zeros([self.queue_length, self.queue_size, self.dim, 2], dtype=np.float32)
        self.queue_ptr = 0
        self.queue_counter = np.zeros(self.queue_length,dtype=np.int32)
        self.save_path = save_path
        self.mining_start = 1
        self.mining_pool_size = min(40000, len(dataset.train_sat_cover_list))
        self.mining_save_size = 100
        self.choice_pool = range(self.mining_save_size)
        self.mining_save = np.zeros([self.queue_length, self.mining_save_size],dtype=np.int32)
        self.mode = mode
    ...
    def generate_indices_sim(self, device):
        self.queue_ptr = 0
        ...
        assert self.queue.dtype == np.float32, f'High precision explodes the memory, float32 only supported, got dtype {self.queue.dtype}'

        if self.epoch >= self.mining_start:
            assert self.mining_pool_size <= self.queue_length
            product_index = np.empty((len(self.queue), self.mining_save_size), dtype=np.int32)
            mining_pool = np.array(random.sample(range(len(self.dataset.train_sat_cover_list)), self.mining_pool_size),dtype=np.int32)
            with torch.no_grad():
                # f_obj, product_train, filename = matmul(self.queue[:,:,:,1].mean(axis=1), np.transpose(self.queue[mining_pool,:,:,0].mean(axis=1)))
                start = 0
                for i in range(len(self.queue)//10, len(self.queue), len(self.queue)//10):
                    end = i
                    product_train = torch.matmul(torch.from_numpy(self.queue[start:end,:,:,1]).pin_memory().to(device).mean(dim=1), torch.t(torch.from_numpy(self.queue[mining_pool,:,:,0]).pin_memory().to(device).mean(dim=1)))
                    product_index[start:end] = torch.argsort(product_train, dim=1).detach().cpu().numpy()[...,-self.mining_save_size:]
                    del product_train
                    start = end
                # product_index = product_index.detach().cpu().numpy()
            
            temp_ix = random.choice(range(self.mining_pool_size))
            # update mining pool
            for i in range(product_index.shape[0]):
                # product_index = np.argsort(product_train[i])
                indexes = product_index[i, -self.mining_save_size:]
                indexes[indexes >= self.mining_pool_size] = temp_ix
                self.mining_save[i, :] = mining_pool[indexes]
                # self.mining_save[i, :] = mining_pool[product_index[-self.mining_save_size:]]
            # f_obj.close()
            # os.remove(f'{filename}.hdf5')
        ...
        self.current_indices = np.array(self.current_indices, dtype=int)
        assert len(self.current_indices) == self.current_size
        sys.stdout.write('sampler updated!\n')
        # print('sampler updated!')

    def update_epoch(self, device):
        # if self.epoch >= self.mining_start:
        # print(self.rank, self.queue_ptr)
        if device is None:
            torch.distributed.barrier()
            # self.load(self.save_path)
            self.queue_ptr = 0
            self.queue_counter = np.load(os.path.join(self.save_path, 'queue_counter.npy')).astype(np.int32)
            self.queue = np.load(os.path.join(self.save_path, 'queue.npy')).astype(np.float32)
            self.current_indices = np.load(os.path.join(self.save_path, 'current_indices.npy')).astype(self.current_indices.dtype)
            return
        self.generate_indices_sim(device)
        # if self.rank == 0:
        np.save(os.path.join(self.save_path, 'queue.npy'), self.queue)
        np.save(os.path.join(self.save_path, 'queue_counter.npy'), self.queue_counter)
        np.save(os.path.join(self.save_path, 'current_indices.npy'), self.current_indices)
        torch.distributed.barrier()

    def load(self, path):
        self.mining_start = 0
        self.queue_counter = np.load(os.path.join(path, 'queue_counter.npy')).astype(np.int32)
        self.queue = np.load(os.path.join(path, 'queue.npy')).astype(np.float32)
    ...
    def __iter__(self) :
        ...
        # subsample
        self.indices_out = self.current_indices[indices]
        indices = indices[self.rank:self.current_size:self.num_replicas]
        # assert len(indices) == self.num_samples
        # print(indices)
        indices_out = self.current_indices[indices]
        # print(self.rank, len(indices), len(indices_out))
...
if __name__ == "__main__":
    ...
    args = Args()
    dataset = GAMa(mode='train', args=args, print_bool=True)
    
    sampler = DistributedMiningSamplerVigor(dataset, num_replicas=1, rank=0)

    sampler.load('../result_debug')
    sampler.set_epoch(18)
    sampler.generate_indices_sim('cuda:0')