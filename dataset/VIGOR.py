...
class VIGOR(torch.utils.data.Dataset):
    def __init__(self, mode = '', root = '/home/ma293852/Project/dataset/VIGOR/', same_area=True, print_bool=False, polar = '', args=None):
        ...
        if same_area:
            if args.city == "all":
                ...
                self.train_city_list = [args.city]
                self.test_city_list = [args.city] 
        else:
...
def train_transform_fov(size, fov):
    return transforms.Compose([
        transforms.RandomPosterize(bits=2),
        transforms.RandomEqualize(p=0.5),
        transforms.RandomAutocontrast(p=0.5),
...
class VIGOR_G(torch.utils.data.Dataset):
    def __init__(self, mode = '', root = '/home/ma293852/Project/dataset/VIGOR/', same_area=True, print_bool=False, polar = '', args=None):
        super(VIGOR_G, self).__init__()
        ...
        if args.fov != 0:
            self.transform_query = input_transform_fov(size=self.grd_size,fov=args.fov)
            self.train_transform = train_transform_fov(size=self.grd_size,fov=args.fov)
        else:
            self.transform_query = input_transform(size=self.grd_size)
    ...
    def __getitem__(self, idx, debug=False):
        if 'train' in self.mode:
            idx = idx % self.train_data_size
            img_query = Image.open(self.train_list[idx])
            # img_query = self.transform_query(img_query)
            img_query1 = self.train_transform(img_query)
            img_query2 = self.train_transform(img_query)

            return img_query1, img_query2, torch.tensor(idx), 0, 0, 0
        elif 'scan_val' in self.mode:
            img_query = random.choice(self.test_list)
            img_query = Image.open(img_query)
            img_query = self.transform_query(img_query)
            
            return img_query, img_query, 0, 0, 0, 0

        elif 'test_query' in self.mode or 'test_reference' in self.mode:
            img_query = Image.open(self.test_list[idx])
            img_query = self.transform_query(img_query)
...
if __name__ == "__main__":
    class Args:
        ...
        city="SanFrancisco"
    args = Args()
    dataset = VIGOR_G(mode='train', args=args)
    dataloader = torch.utils.data.DataLoader(dataset)

    for output in dataloader:
        print(torch.mean((output[0] == output[1]).to(torch.float)))
        break