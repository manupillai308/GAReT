...
# query flops: 257.11026632
    with torch.cuda.device(0):
        macs_1, params_1 = get_model_complexity_info(model.query_net, input_res=(3, size_grd[0], size_grd[1]),input_constructor=lambda x: {'x':torch.randn((38, 3, size_grd[0], size_grd[1])).cuda()},
...
if __name__ == "__main__":
    ...
    import time
    with torch.no_grad():
        model = TransGeo(args=args).cuda()
        print("Trainable params:", sum(
        p.numel() for p in model.parameters() if p.requires_grad))
        torch.cuda.reset_peak_memory_stats(device=0)
        s = time.time()
        model.query_net(torch.randn(38, 3, 216, 384, dtype=torch.float32).cuda())
