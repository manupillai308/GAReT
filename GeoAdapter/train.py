...
from dataset.GAMa import GAMa
# from dataset.SeqGeo import SeqGeo
...
parser.add_argument('--type', choices=['asym', 'sym1', 'sym2', 'avg'], required=True)

parser.add_argument('--cross', action='store_true',
                    help='use cross area')

parser.add_argument('--dataset', default='vigor', type=str,
                    help='vigor, cvusa, cvact, bdd, gama, seqgeo')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # gpu_query = torch.device(0)
    # gpu_ref = torch.device(1) if torch.cuda.device_count() > 1 else torch.device(0)
    # args.ngpus_per_node = ngpus_per_node
    # if not args.cross:
    #     print(f"Info: training on {args.city} city dataset split/s")

    # if args.gpu is not None:
    #     print(f"Use GPU: {gpu_query} and {gpu_ref} for training")
    # # create model
    ...
    if args.distributed:
        ...
        dist.init_process_group(backend=args.dist_backend, init_method=args.dist_url,
                                world_size=args.world_size, rank=args.rank, timeout=timedelta(seconds=7200000))
    # create model
    print("=> creating model '{}'")

    model = TransGeo(num_frames=8, args=args)
    # model.reference_net.to(gpu_ref)
    # model.query_net.to(gpu_query)

    
    ckpt_path='/home/ma293852/Project/TransGeo2022/result_gama_full/model_best.pth.tar'
    ...
    #     model = model.cuda(args.gpu)
        ...
        raise RuntimeError("Only Distributed is supported!")
    ...
    if args.dataset == 'vigor':
        ...
        dataset = BDD
        mining_sampler = DistributedMiningSamplerVigor
    elif args.dataset == 'gama':
        dataset = GAMa
        mining_sampler = DistributedMiningSamplerVigor
    elif args.dataset == 'seqgeo':
        # dataset = SeqGeo
        mining_sampler = DistributedMiningSamplerVigor
    ...
    val_query_loader = torch.utils.data.DataLoader(
        val_query_dataset,batch_size=64, shuffle=False,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # if args.mining:
        #     train_sampler.update_epoch(args.gpu)
        adjust_learning_rate(optimizer, epoch, args)
        
        # torch.cuda.empty_cache()
        # gc.collect()
        # train for one epoch
        train(train_loader, model, criterion, optimizer, epoch, args, train_sampler=train_sampler)
        ...
        if args.gpu == 0:
            if args.mining:
                train_sampler.update_epoch(0)
        elif args.gpu == 1 or not args.multiprocessing_distributed:
        ...
        if args.gpu != 0:
            if args.mining:
                train_sampler.update_epoch(None)
        ...
        if not args.multiprocessing_distributed or args.gpu == 1:
            if args.distributed:
                state_dict = model.module.state_dict()
            else:
                state_dict = model.state_dict()

            # print('saving checkpoint', acc1, best_acc1)
...
def validate(val_query_loader, val_reference_loader, model, args):
    ...
    if args.distributed:
        if args.gpu is not None:
            torch.cuda.set_device(args.gpu)
            model_query.to(args.gpu)
            model_reference.to(args.gpu)
    ...
    with torch.no_grad():
        ...
        for i, (images, indexes, atten) in enumerate(val_reference_loader):
            ...
            if args.type == 'avg':
                reference_embed = reference_embed.reshape(B, T, args.dim).mean(dim=1)
        ...
        for i, (images, indexes, labels) in enumerate(val_query_loader):
            ...
            if args.type == 'avg':
                query_embed = query_embed.reshape(B, T, args.dim).mean(dim=1)
...
def accuracy(query_features, reference_features, query_labels, topk=[1,5,10]):
    ...
    sys.stdout.write('Percentage-top1:{}, top5:{}, top10:{}, top1%:{}, time:{}\n'.format(results[0], results[1], results[2], results[-1], time.time() - ts))
    return results[:2]
