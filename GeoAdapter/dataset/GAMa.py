...
# Image.open = open_with_retry

class Patchify:
    def __init__(self, patch_size, overlap=False):
        self.patch_size = patch_size
        self.overlap = overlap
    
    def __call__(self, sample):
        patches = patchify(np.array(sample), patch_size=self.patch_size, step=self.patch_size[0] if not self.overlap else self.overlap)
        patches = patches.squeeze().reshape(49, *self.patch_size)
        return patches
...
class ToTensor:
    def __init__(self):
        pass
    def __call__(self, sample):
        return torch.cat([transforms.functional.to_tensor(pic).unsqueeze(0) for pic in sample])
...
class GAMa(torch.utils.data.Dataset):
    def __init__(self, mode = '', root = '/home/c3-0/sarucrcv/geo3/BDD100k_Big/Ground', same_area=True, print_bool=False, polar = '', gama_root='/home/ma293852/Project/dataset/GAMa', args=None):
        ...
        self.train_sat_list = []
        self.train_sat_index_dict = dict()
        ...
        self.train_list = []
        self.train_label = []
        self.train_sat_cover_dict = dict()
        ...
        self.train_list = np.array(self.train_list).astype(np.string_)
        ...
        self.test_list = np.array(self.test_list).astype(np.string_)
        self.test_label = np.array(self.test_label)
        self.test_delta = np.array(self.test_delta)
        if print_bool:
            print('Test grd loaded, data size: {}'.format(self.test_data_size))

        self.train_sat_cover_dict = pd.Series(self.train_sat_cover_dict)
        self.train_sat_index_dict = pd.Series(self.train_sat_index_dict)
        self.test_sat_cover_dict = pd.Series(self.test_sat_cover_dict)
        self.test_sat_index_dict = pd.Series(self.test_sat_index_dict)
    ...
    def load_img(self, imgs, transform):
        out = [transform(open_with_retry(img)).unsqueeze(0) for img in imgs]
    ...
    def __getitem__(self, index, debug=False):
        if 'train' in self.mode:
            ...
            img_reference = open_with_retry(self.train_sat_list[self.train_label[idx][0]]).convert('RGB')
            img_reference = self.transform_reference(img_reference)
            if self.args.crop:
                atten_sat = open_with_retry(os.path.join(self.args.resume.replace(self.args.resume.split('/')[-1],''),'attention','train', str(idx)+'.png')).convert('RGB')
            ...
            img_reference = open_with_retry(self.test_sat_list[index]).convert('RGB')
            img_reference = self.transform_reference(img_reference)
            ...
            img_reference = self.transform_reference(img_reference)
            if self.args.crop:
                atten_sat = open_with_retry(os.path.join(self.args.resume.replace(self.args.resume.split('/')[-1],''),'attention','val', str(index)+'.png')).convert('RGB')
...
if __name__ == "__main__":
    ...
    for output in dataloader:
        ...
        # grid_img = torchvision.utils.make_grid(output[1].squeeze(), nrow=7)
