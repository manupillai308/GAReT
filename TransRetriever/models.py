...
class TSP_net(nn.Module): 
    ...
    def forward(self, x, labels, deterministic=True):
        
        if labels is not None:
            labels_indx = labels.argmax(dim=2)
            labels_max = labels.max(dim=2)[0]
            
        bsz, ngroups, nb_nodes, nfeats = x.size()
        ...
        # input embedding layer
        h = self.input_emb(rearrange(x, 'b g n f -> b (g n) f')) # size(h)=(bsz, nb_nodes, dim_emb)
        h = rearrange(h, 'b (g n) f -> b g n f', b=bsz, g=ngroups, n=nb_nodes) # size(h)=(bsz, ngroups, nb_nodes, dim_emb)
        
        for i in range(ngroups): # add PE group wise
            h[:, i] = h[:, i] +  self.PE[i].expand(bsz, nb_nodes, self.dim_emb)
        
        
        h = rearrange(h, 'b g n f -> b (g n) f') # size(h) = (bsz, ngroups*nb_nodes, dim_emb)
        ...
        h_encoder, h_z = h_encoder[:, :-1], h_encoder[:, -1]
        
        h_encoder = rearrange(h_encoder, 'b (g n) f -> b g n f', b=bsz, g=ngroups, n=nb_nodes) # size(h_encoder) = (bsz, ngroups, nb_nodes+1, dim_emb)
        ...
        h_encoder_grp = h_encoder[:, 0] # 0th group

        # key and value for decoder    
        K_att_decoder = self.WK_att_decoder(h_encoder_grp) # size(K_att)=(bsz, nb_nodes+1, dim_emb*nb_layers_decoder)
        V_att_decoder = self.WV_att_decoder(h_encoder_grp) # size(V_att)=(bsz, nb_nodes+1, dim_emb*nb_layers_decoder)
        
        # input placeholder that starts the decoding
        h_start = h_z + self.PE[0].repeat(bsz,1) # size(h_start)=(bsz, dim_emb)
...
#         mask_visited_nodes[zero_to_bsz, idx_start_placeholder] = True
        ...
        for t in range(ngroups):
            ...
            prob_next_node = self.decoder(h_t, K_att_decoder, V_att_decoder, mask=None) # size(prob_next_node)=(bsz, nb_nodes+1)
...
#             ProbOfChoices = prob_next_node[zero_to_bsz, idx]
            ProbOfChoices.append(torch.log(prob_next_node))  # size(query)=(bsz,)
            # update tour
            tours.append(idx)
            
            if labels is not None:
                gt_idx = labels_indx[:, t].clone()
                gt_idx[labels_max[:, t] == 0] = idx[labels_max[:, t] == 0]
                idx = gt_idx
            # update embedding of the current visited node
            h_t = h_encoder_grp[zero_to_bsz, idx, :] # size(h_start)=(bsz, dim_emb)
...
#             mask_visited_nodes[zero_to_bsz, idx] = True
            if t < ngroups-1:
                h_encoder_grp = h_encoder[:, t+1] # 0th group
                K_att_decoder = self.WK_att_decoder(h_encoder_grp)
                V_att_decoder = self.WV_att_decoder(h_encoder_grp)
            
            
        # logprob_of_choices = sum_t log prob( pi_t | pi_(t-1),...,pi_0 )
        ProbOfChoices = torch.stack(ProbOfChoices,dim=1) # size(sumLogProbOfActions)=(bsz,)

        # convert the list of nodes into a tensor of shape (bsz,num_cities)
        tours = torch.stack(tours,dim=1) # size(col_index)=(bsz, nb_nodes)
        
        return tours, ProbOfChoices